...
class LoginMember extends StatefulWidget{
  ...
  @override
  _LoginMemberState createState() => new _LoginMemberState();
}
...
class _LoginMemberState extends State<LoginMember>{
  ...
  Future<void> _loginS() async{
    final response = await http.post("http://192.168.100.59/avera/loginmember.php", body:{
      "username": user.text,
      "password": pass.text,
    });
    ...
    if(admin.length==0){
      ...
      Navigator.pushReplacement(context, new MaterialPageRoute(builder: (context) => HomeMember(username:username, id:id, membership:membership, nama:nama, gender:gender, email:email, no_telp:no_telp, alamat:alamat, tgl_join:tgl_join, password:password,)));
      setState(() {
